...
const project_update = async (data, id) => {
  const project = {
    title: data.get('title'),
    description: data.get('description'),
    payment: data.get('payment'),
  ...
  const res = await axios.put(`${process.env.REACT_APP_HOST}api/projects/update_admin/${id}`,project);
  return res.status;
}
export { post_project, projects_get, project_delete, apply_project, project_update };